...
var parseMetadata = function (metadata) {
    ...
    if (typeof authors === 'object') {
        authors = authors.map(function (author) { return lodash_1.default.get(author, ['_']); });
    }
    var publisher = lodash_1.default.get(metadata[0], ['dc:publisher', 0]);
    var rawIdentifiers = lodash_1.default.get(metadata[0], ['dc:identifier'], []);
    var identifiers = rawIdentifiers.reduce(function (result, identifier) {
        var identifierType = identifier.$['opf:scheme'] || identifier.$.id || null;
        var value = identifier._ || null;
        if (!value) {
            return result;
        }
